...
OUTPUT_FILE_EWAS="ckdgen-EWAS-pheno-%STUDY%-%TIMESTAMP%.out.csv"
...
COLUMN_HEIGHT_CREA_SERUM="height"
COLUMN_HEIGHT_FOLLOWUP="height_fu"
...
OUTPUT_FILE_EWAS=`echo ${OUTPUT_FILE_EWAS} | sed s/%STUDY%/${STUDY_NAME}/g | sed s/%TIMESTAMP%/${TIMESTAMP}/g`
OUTPUT_FILE_EWAS="${OUTPUT_DIRECTORY}/${OUTPUT_FILE_EWAS}"
echo "Writing EWAS output as CSV to: ${OUTPUT_FILE_EWAS}" | tee -a ${LOG_FILE}
rm -f ${OUTPUT_FILE_EWAS}
touch ${OUTPUT_FILE_EWAS}
